...
| opencode Config | - | ✅ | `.opencode/opencode.json` with agents + commands |
...
17. **Web search fallback** — Wikipedia API fallback when DDGS returns 0 results. Search cache persisted to `agent_memory/search_cache.json`. All tiers (DDGS → Wikipedia → cache) integrated into `WebSearch.search()`. Also added `.opencode/` config with web-search agent and commands.
