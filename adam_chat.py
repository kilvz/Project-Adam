...
class CognitiveAgent:
    ...
    def _lora_train_step(self):
        """E26: Reward-weighted curriculum learning on recent episodes."""
        episodes = self.episodic_memory.episodes
        if len(episodes) < 5:
            return

        # curriculum: sort by reward descending, skip negative-reward
        ...
        for i, text in enumerate(texts):
            ...
            if not torch.isnan(loss):
                # reward-weighted: higher reward → stronger gradient
...
# ═══════════════════════════════════════════════════════════════════════
# D21: Voice mode (faster-whisper ASR + edge-tts TTS)
# ═══════════════════════════════════════════════════════════════════════
...
class VoiceMode:
    ...
    def speak(self, text):
        import edge_tts, asyncio, miniaudio, sounddevice as sd, numpy as np
        async def _speak():
