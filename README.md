...
`external.ai/zen/v1/chat/completions` with `External`. No API key required. Falls back to local model on failure. In `auto` mode, low-tier hardware automatically uses the API.
