...
class _EncoderBase(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, dtype=torch.float32,
                 hardware_tier="low"):
        super().__init__()
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, latent_dim * 2, dtype=dtype),
        )
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, hidden_dim, dtype=dtype),
            nn.ReLU(),
            nn.Linear(hidden_dim, input_dim, dtype=dtype),
        )
    ...
    def compute_loss(self, x, rpe=0.0):
        ...
        task_weight = 1.0 if self.hardware_tier in ("mid", "high") else 0.1
...
class VisionEncoder(_EncoderBase):
    def __init__(self, input_dim=2048, latent_dim=128, dtype=torch.float32,
                 hardware_tier="low"):
        super().__init__(input_dim, 512, latent_dim, dtype, hardware_tier)


class AudioEncoder(_EncoderBase):
    def __init__(self, input_dim=1024, latent_dim=64, dtype=torch.float32,
                 hardware_tier="low"):
        super().__init__(input_dim, 256, latent_dim, dtype, hardware_tier)
